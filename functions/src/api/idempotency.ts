...
export const withIdempotency = <T, R>(
    ...
    return async (data: T, context: functions.https.CallableContext): Promise<R> => {
        ...
        const idempotencyKey = payload?.idempotencyKey || payload?._idempotencyKey
        ...
        try {
            return await db.runTransaction(async (t) => {
                ...
                // We need to execute the handler OUTSIDE the transaction if it performs other DB ops?
                // Firestore transactions require all reads before writes. 
                // Running complex business logic inside a transaction can be tricky if it does external calls.
                // However, to ensure atomicity of the "Lock", we must return here and let the handler run? No.
                // Standard pattern: 
                // - Transaction: Lock (create PROCESSING)
                // - Execute Logic
                ...
                // Correction: We just use the transaction to SET PROCESSING. 
        ...
        // Retry-safe implementation utilizing specific return values is complex in TS inside the transaction.
        // Let's go with a simpler "Check-then-Create" atomic operation via transaction.

        let existingResult: R | undefined
        ...
        await db.runTransaction(async (t) => {
            ...
            if (doc.exists) {
                ...
                if (record.status === 'PROCESSING') {
                    throw new functions.https.HttpsError('aborted', 'Request in progress')
                }
            }
