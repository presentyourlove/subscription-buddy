 ...
 */
export const acquireLock = async (resourceId: string, ttlMs = 5000): Promise<string | null> => {
 ...
 */
export const releaseLock = async (resourceId: string, lockId: string): Promise<void> => {
    const db = admin.firestore()
    const lockRef = db.collection(LOCK_COLLECTION).doc(resourceId)
    ...
    try {
        await db.runTransaction(async (t) => {
            ...
            if (data && data.lockId === lockId) {
                ...
                logger.warn(`Lock release failed: Ownership changed for ${ resourceId }`)
            }
        })
