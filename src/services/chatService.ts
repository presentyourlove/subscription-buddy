...
class ChatService {
     ...
     */
    subscribeToMessages(groupId: string, callback: (messages: Message[]) => void, onError: (error: any) => void) {
        const q = query(
     ...
     * Rate another user
     */
    async rateUser(groupId: string, targetUserId: string, score: number, reviewerId: string): Promise<void> {
        await runTransaction(db, async (transaction) => {
            const chatRef = doc(db, COLLECTIONS.CHATS, groupId)
            ...
            if (!isClosed) {
                ...
                if (groupDoc) {
                    ...
                    try {
                        ...
                        if (gSnap.exists() && (gSnap.data() as any).status === GROUP_STATUS.CLOSED) isClosed = true
                    } catch (e) {
                        // ignore error
                    }
                }
