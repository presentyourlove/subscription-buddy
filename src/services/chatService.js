...
import { collection, doc, getDoc, getDocs, setDoc, updateDoc, addDoc, onSnapshot, query, orderBy, serverTimestamp, arrayUnion, arrayRemove, runTransaction } from 'firebase/firestore';
...
class ChatService {
    ...
    subscribeToMessages(groupId, callback, onError) {
        const q = query(collection(db, COLLECTIONS.CHATS, groupId, COLLECTIONS.MESSAGES), orderBy('createdAt', 'asc'));
    ...
    async rateUser(groupId, targetUserId, score, reviewerId) {
        await runTransaction(db, async (transaction) => {
            ...
            const data = chatDoc.data();
            if (data.ratings && data.ratings[reviewerId] && data.ratings[reviewerId][targetUserId]) { // Corrected check
    ...
    async checkPendingReviews(user) {
        ...
        const qChats = query(collection(db, COLLECTIONS.CHATS), where('participants', 'array-contains', user.uid));
        const chatSnaps = await getDocs(qChats);
        // Strategy 2: Check groups where I am the Host
        const qGroups = query(collection(db, COLLECTIONS.GROUPS), where('hostId', '==', user.uid), where('status', '==', GROUP_STATUS.CLOSED));
            ...
            if (!isClosed) {
                ...
                if (groupDoc) {
                    ...
                    try {
                        ...
                        if (gSnap.exists() && gSnap.data().status === GROUP_STATUS.CLOSED)
                            isClosed = true;
                    }
                    catch (e) {
                        // ignore error
